# Kaplan Violante - impConsumption
...
# log impConsumption
f030507_KV$logimpConsumption03 <- log(f030507_KV$impConsumption03)
f030507_KV$logimpConsumption05 <- log(f030507_KV$impConsumption05)
f030507_KV$logimpConsumption07 <- log(f030507_KV$impConsumption07)

f091113_KV$logimpConsumption09 <- log(f091113_KV$impConsumption09)
f091113_KV$logimpConsumption11 <- log(f091113_KV$impConsumption11)
f091113_KV$logimpConsumption13 <- log(f091113_KV$impConsumption13)
...
# drop NA 
#f030507_KV <- f030507_KV[complete.cases(f030507_KV[, c("logy03","logy05","logy07","logimpConsumption03","logimpConsumption05","logimpConsumption07")]),]
#f091113_KV <- f091113_KV[complete.cases(f091113_KV[, c("logy09","logy11","logy13","logimpConsumption09","logimpConsumption11","logimpConsumption13")]),]
...
regressionData_030507 <- reshape(f030507_KV, idvar=c("uniqueID","primarySamplingUnit","stratification"), direction="long", 
                                 varying=list(black=c(grep("black", colnames(f030507_KV))),
                                              ...
                                              logimpConsumption=c(grep("logimpConsumption",colnames(f030507_KV))),
                                 ...
                                 v.names = c("black","educ","employed",
                                             ...
                                             "other","logy","region","retired","unemployed","white","year","logimpConsumption","age","poorHTM","richHTM"), #,"bigCity"),
                                 times=c("03", "05","07"))
...
regressionData_091113 <- reshape(f091113_KV, idvar=c("uniqueID","primarySamplingUnit","stratification"), direction="long", 
                                 varying=list(black=c(grep("black", colnames(f091113_KV))),
                                              ...
                                              logimpConsumption=c(grep("logimpConsumption",colnames(f091113_KV))),
...
regressionData_030507 <- regressionData_030507 %>% add_residuals(income_030507.lm)	
names(regressionData_030507)[names(regressionData_030507)=="resid"] <-"resIncome"



income_091113.lm = rq(logy ~ year +
                        yob +
                        educ +
                        white +
                        black +
                        ...
                        retired*year +
                        region*year
                      , regressionData_091113, tau = seq(0, 1, by = 0.2)) 

regressionData_091113 <- regressionData_091113 %>% add_residuals(income_091113.lm)	
names(regressionData_091113)[names(regressionData_091113)=="resid"] <-"resIncome"

impConsumption_030507.lm = rq(logimpConsumption ~ year +
                             yob +
                             educ +
                             white +
                             black +
                             ...
                             region*year
                           , regressionData_030507, tau = seq(0, 1, by = 0.2)) 
regressionData_030507 <- regressionData_030507 %>% add_residuals(impConsumption_030507.lm)	
names(regressionData_030507)[names(regressionData_030507)=="resid"] <-"resimpConsumption"


impConsumption_091113.lm = rq(logimpConsumption ~ year +
                             yob +
                             educ +
                             white +
                             black +
                             ...
                             region*year,
                           regressionData_091113, tau = seq(0, 1, by = 0.2)) 
regressionData_091113 <- regressionData_091113 %>% add_residuals(impConsumption_091113.lm)	
names(regressionData_091113)[names(regressionData_091113)=="resid"] <-"resimpConsumption"


covData_030507 = regressionData_030507[ , c("uniqueID","primarySamplingUnit","stratification","longWeight", "quintile","year","resimpConsumption","resIncome")]
covData_091113 = regressionData_091113[ , c("uniqueID","primarySamplingUnit","stratification","longWeight","quintile","year","resimpConsumption","resIncome")]

covData_030507 <- reshape(covData_030507,idvar="uniqueID",direction="wide",v.names=c("resimpConsumption","resIncome","longWeight"),timevar="year")
covData_091113 <- reshape(covData_091113,idvar="uniqueID",direction="wide",v.names=c("resimpConsumption","resIncome","longWeight"),timevar="year")

covData_030507$dct <- covData_030507$resimpConsumption.2005 - covData_030507$resimpConsumption.2003
...
covData_091113$dct <- covData_091113$resimpConsumption.2011 - covData_091113$resimpConsumption.2009
...
#          cov((covData_030507$resimpConsumption.2005[,i] - covData_030507$resimpConsumption.2003[,i]),(covData_030507$resIncome.2007[,i] - covData_030507$resIncome.2005[,i]))/cov((covData_030507$resIncome.2005[,i] - covData_030507$resIncome.2003[,i]),(covData_030507$resIncome.2007[,i] - covData_030507$resIncome.2005[,i]))
#   )
...
#          cov((covData_091113$resimpConsumption.2011[,i] - covData_091113$resimpConsumption.2009[,i]),(covData_091113$resIncome.2013[,i] - covData_091113$resIncome.2011[,i]))/cov((covData_091113$resIncome.2011[,i] - covData_091113$resIncome.2009[,i]),(covData_091113$resIncome.2013[,i] - covData_091113$resIncome.2011[,i]))
#   )
...
#   impConsumption_030507.lm, impConsumption_091113.lm, income_030507.lm, income_091113.lm)

# MPC
print("MPC as 07")
for (i in c(1,2,3,4,5)){
  mdl <- ivreg(dct[,i] ~ dyt[,i], ~ dytplus1[,i], x=TRUE, data=covData_030507)
  assign(paste0("MPC_030507_q",i),
         mdl$coefficients[2]
  )
...
print("MPC as 13")
for (i in c(1,2,3,4,5)){
  mdl = ivreg(dct[,i] ~ dyt[,i], ~ dytplus1[,i], x=TRUE, data=covData_091113)
  assign(paste0("MPC_091113_q",i),
         mdl$coefficients[2]
  )
  temp <- anderson.rubin.ci(mdl)
